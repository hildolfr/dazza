...
export default class MediaTracker {
    ...
    initialize(config) {
        try {
            ...
            // Create indexes
            this.db.exec(`
                CREATE INDEX IF NOT EXISTS idx_media_url ON media_history(url);
                CREATE INDEX IF NOT EXISTS idx_media_room ON media_history(room_id);
                CREATE INDEX IF NOT EXISTS idx_media_added_by ON media_history(added_by);
                CREATE INDEX IF NOT EXISTS idx_media_first_seen ON media_history(first_seen);
            `);
    ...
    async recordMediaPlay(media) {
        ...
        try {
            ...
            if (existing) {
                ...
                `).run(now, title || existing.title, queueby || existing.added_by, url, this.roomId);
                ...
                this.db.prepare(`
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                `).run(url, title || 'Unknown', queueby || 'Unknown', this.roomId, now, now);
            ...
            if (existing) {
                ...
                this.db.prepare(`
                    ...
                    SET queue_count = queue_count + 1,
                        ...
                        updated_at = CURRENT_TIMESTAMP
                    WHERE url = ? AND room_id = ?
                `).run(title || existing.title, queueby || existing.added_by, url, this.roomId);
